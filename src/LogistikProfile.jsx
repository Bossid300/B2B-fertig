  ...
  useEffect(() => {
    try {
      ...
      const freshData = currentProj || { 
        ...
        name: ticketName || '', project_name: '', vorname: '', nachname: '', website: '', city: '', role: 'Logistik',
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-slate-950 border border-slate-900 rounded-3xl font-mono text-white shadow-2xl relative">
      ...
      <div className="h-44 w-full rounded-2xl overflow-hidden bg-slate-900 border border-slate-800/60 relative mb-6 group">
        <img src={slides[currentSlide]} alt={`Logistik Slide ${currentSlide + 1}`} className="w-full h-full object-cover opacity-20 transition-all duration-500" />
        ...
        <div className="absolute bottom-4 left-4 z-10">
          <span className="text-[8px] bg-blue-500/10 border border-blue-500 text-blue-400 font-bold px-2 py-0.5 rounded-md uppercase tracking-widest shadow-[0_0_10px_rgba(59,130,246,0.2)]">
            🚛 CARGO & SHUTTLE LOGISTICS
          </span>
          <h1 className="text-lg font-black uppercase text-white tracking-wide mt-1">{localFields.project_name || 'Unbenannter Logistik-Service'}</h1>
        </div>

        <div className="absolute bottom-4 right-4 z-10 flex gap-1">
          {slides.map((_, idx) => (
            <div key={idx} className={`h-1 transition-all duration-300 ${idx === currentSlide ? 'w-4 bg-blue-400' : 'w-1 bg-slate-700'}`} />
          ))}
        </div>
      </div>
          ...
          {isEditing ? (
            <div className="bg-slate-950 border border-slate-900 rounded-2xl p-5 space-y-4 shadow-xl">
              ...
              <div className="space-y-2 pb-3 border-b border-slate-900/60 font-mono">
                <span className="text-[8px] text-blue-400 block tracking-widest font-black">// PROFILMEDIEN & SLIDER-CONFIG</span>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[7px] text-slate-600 uppercase block font-bold">Profilbild / Avatar URL</label>
                    <input type="text" name="avatarUrl" value={localFields.avatarUrl || ''} onChange={handleInplaceChange} className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2 py-1 text-white text-[10px] outline-none focus:border-blue-500/30 font-mono" placeholder="URL für Crew-Avatar" />
                  </div>
                  <div className="space-y-1">
